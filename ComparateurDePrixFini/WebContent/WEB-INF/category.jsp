        ...
        <div class="control-group">
            ...
            <ul class="categories-filter animate-dropdown">
                <li class="dropdown">
                    ...
                    <a class="dropdown-toggle"  data-toggle="dropdown">Categories <b href="category.html" class="caret"></b></a>
	...
	<div class="dropdown dropdown-cart">
		...
		<ul class="dropdown-menu">
			...
			<c:forEach var="product" items="${productstocompare }">
				<div class="cart-item product-summary">
					<div class="row">
						<div class="col-xs-4">
							<div class="image">
								<a href="detail.html"><img src="${product.imglink}" alt=""></a>
							</div>
						</div>
						<div class="col-xs-7">
						...
						<div class="col-xs-1 action">
							<a onclick="buttonCompare(${product.id})"><i class="fa fa-trash"></i></a>
						</div>
					</div>
				</div><!-- /.cart-item -->
			</c:forEach>
			...
			<div class="clearfix">
				...
				<a href="compare" id="lienid" class="btn btn-upper btn-primary btn-block m-t-20">Comparer</a>	
			</div><!-- /.cart-total-->
...
<div class="side-menu animate-dropdown outer-bottom-xs">
    ...
    <nav class="yamm megamenu-horizontal" role="navigation">
        ...
        <c:forEach var="categorie" items="${categories }">
    		...
    		<c:forEach var="categoriechild" items="${categories }" varStatus="status">
                <c:if test="${categoriechild.pid eq categorie.id }" > 
                    <c:set var="test" value="${test + 1 }"/>
                    </c:if>
                   </c:forEach>

                    <fmt:parseNumber var="nb" integerOnly="true" type="number" value="${test / 4}" />
                    <c:if test="${nb < 1 }"><c:set var="nb" value="1"/></c:if>
                    
            <li class="dropdown menu-item">
                <a href="category?id=${categorie.id }" <c:if test="${test ne 0 }">class="dropdown-toggle" data-toggle="dropdown"</c:if> ><i class="icon fa ${ categorie.icon} fa-fw"></i>${categorie.name }</a>
    ...
    <li class="yamm-content">
        ...
        <c:set var="compteur" value="0"/>
            ...
            <div class="col-sm-12 col-md-3">
                ...
                <c:if test="${categoriechild.pid eq categorie.id }" > 
                    <c:set var="compteur" value="${compteur + 1 }"/>
                    <c:if test="${compteur > nb and compteur <= (nb * 2)}" >
                    <li><a href="category?id=${categoriechild.id}">${categoriechild.name }</a></li>
                    </c:if>
                    </c:if>
                   </c:forEach>     
            ...
            <div class="col-sm-12 col-md-3">
                ...
                <c:if test="${categoriechild.pid eq categorie.id }" > 
                    <c:set var="compteur" value="${compteur + 1 }"/>
                    <c:if test="${compteur <= (nb * 3) and compteur > (nb * 2)}" >
                    <li><a href="category?id=${categoriechild.id}">${categoriechild.name }</a></li> 
            ...
            <div class="col-sm-12 col-md-3">
                ...
                <c:if test="${categoriechild.pid eq categorie.id }" > 
                    <c:set var="compteur" value="${compteur + 1 }"/>
                    <c:if test="${compteur > (nb * 3) and compteur <= test}" >
                    <li><a href="category?id=${categoriechild.id}">${categoriechild.name }</a></li> 
                    </c:if>
                    </c:if>
                   </c:forEach>   
...
<!-- ========================================= SECTION – HERO : END ========================================= -->
	...
	<ul class="list-inline list-unstyled">
		...
		<li class="prev"><a href="category<c:if test="${empty keyword}">?id=${id}</c:if><c:if test="${!empty keyword}">?searchkeyword=${keyword}</c:if>&page=${prevp}"><i class="fa fa-angle-left"></i></a></li>
		...
		<c:forEach var="i" begin="1" end="${endl}">
   			<li <c:if test="${page eq i }" >class="active"</c:if>><a href="category<c:if test="${empty keyword}">?id=${id}</c:if><c:if test="${!empty keyword}">?searchkeyword=${keyword}</c:if>&page=${i}" >${i }</a></li>	
		</c:forEach>
		<c:if test="${page < endl  }">
		...
		<li class="next"><a href="category<c:if test="${empty keyword}">?id=${id}</c:if><c:if test="${!empty keyword}">?searchkeyword=${keyword}</c:if>&page=${nextp}"><i class="fa fa-angle-right"></i></a></li>
	</ul><!-- /.list-inline -->
			...
			<c:if test="${empty products }"><p>Aucun résultat !</p></c:if>
			<c:forEach var="product" begin="${begin}" end="${end}" items="${products }" varStatus="status">
	...
	<div class="product">		
		<div class="product-image">
			<div class="image">
				<a href="product?id=${product.id }"> <img  src=${product.imglink } style="width:180px;height:200px;object-fit: contain;
    background-repeat: no-repeat;
    background-position: 50% 50%;" alt=""> </a>
			</div><!-- /.image -->			
			...
			<div class="cart clearfix animate-effect">
				<div class="action">
					<ul class="list-unstyled">
						...
						<li class="lnk">
							<a class="add-to-cart" id="${product.id}" onclick="buttonCompare(${product.id});"   title="Compare">
							    <i class="fa fa-retweet"></i>
							</a>
						</li>
					</ul>
	...
	<ul class="list-inline list-unstyled">
		...
		<!-- ============================================== BRANDS CAROUSEL ============================================== -->
<div id="brands-carousel" class="logo-slider wow fadeInUp" style="margin:0px 50px 0px 50px">
...
function add(p1, p2) {
    ...
    if(lien.indexOf(p2)== -1)
    {
       lien=lien+'&'+p1+'='+p2
          while(lien.indexOf('&') == (lien.indexOf('?')+1))
            {
              lien=lien.replace("&","");
            }
    document.getElementById("lienid").href = lien;
    document.getElementById(p2).style.backgroundColor = "#3498DB";
    }
    else
    {
     lien=lien.replace( '&'+p1+'='+p2 ,"")
     lien=lien.replace(p1+'='+p2,"") ;
     while(lien.indexOf('&') == (lien.indexOf('?')+1))
     {
       lien=lien.replace("&","");
     ...
     document.getElementById("lienid").href = lien;
    document.getElementById(p2).style.backgroundColor = "#a8a8a8";
    }
    return (lien)
}
</script>
